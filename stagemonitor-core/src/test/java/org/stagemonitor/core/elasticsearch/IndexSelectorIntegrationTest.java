...
public class IndexSelectorIntegrationTest extends AbstractElasticsearchTest {
	...
	public void testDeleteOldIndices() throws Exception {
		for(ElasticsearchClient esclient : elasticsearchClients) {
			setTime(0L);
			esclient.sendAsJson("POST", "/stagemonitor-metrics-1969.12.30/metrics", "{\"foo\":\"bar\"}");
			esclient.sendAsJson("POST", "/stagemonitor-metrics-1969.12.31/metrics", "{\"foo\":\"bar\"}");
			esclient.sendAsJson("POST", "/stagemonitor-metrics-1970.01.01/metrics", "{\"foo\":\"bar\"}");
			refresh();
			...
			esclient.deleteIndices(indexSelector.getIndexPatternOlderThanDays("stagemonitor-metrics-", 1));
			refresh();
	...
	public void testDeleteOldIndicesUnavailable() throws Exception {
		for(ElasticsearchClient esclient : elasticsearchClients) {
			...
			esclient.sendAsJson("POST", "/stagemonitor-metrics-1970.01.07/metrics", "{\"foo\":\"bar\"}");
			esclient.sendAsJson("POST", "/stagemonitor-metrics-1970.01.09/metrics", "{\"foo\":\"bar\"}");
			esclient.sendAsJson("POST", "/stagemonitor-metrics-1970.01.10/metrics", "{\"foo\":\"bar\"}");
			refresh();
